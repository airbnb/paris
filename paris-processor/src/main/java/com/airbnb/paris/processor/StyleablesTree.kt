...
internal class StyleablesTree(processor: ParisProcessor, private val styleablesInfo: List<BaseStyleableInfo>) : SkyHelper<ParisProcessor>(processor) {
    ...
    internal fun findStyleApplier(viewTypeElement: TypeElement): ClassName {
        ...
        val styleableInfo = styleablesInfo.find { types.isSameType(type, it.viewElementType) }
        if (styleableInfo != null) {
            styleApplierClassName = viewTypeElementToStyleApplierClassName(styleableInfo.elementType.asTypeElement(types))
        } else {
            styleApplierClassName = findStyleApplier(viewTypeElement.superclass.asTypeElement(types))
        }
